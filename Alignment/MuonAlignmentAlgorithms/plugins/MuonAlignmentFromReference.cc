...
MuonAlignmentFromReference::MuonAlignmentFromReference(const edm::ParameterSet& cfg, edm::ConsumesCollector& iC)
  ...
  myOccupancyMap.LoadWeigths(
      "");  //Place a root file containing the weight map for the 250 DTs as a function of X and Y, if you want flat occupancy (and decomment the weight part in MuonResidualsXDOFFitter.cc).
  m_debug = false;
}
...
void MuonAlignmentFromReference::initialize(const edm::EventSetup& iSetup,
  ...
  if (m_gpr_fitter.getDTGeometry())
  {
      std::cout << "DTGeometry* has been passed to MuonResidualsGPRFitter" << std::endl;
      std::cout << "typeid(m_gpr_fitter.getDTGeometry()).name() = " << typeid(m_gpr_fitter.getDTGeometry()).name() << std::endl;
      std::cout << "typeid(m_dtGeometry).name() = " << typeid(m_dtGeometry).name() << std::endl;
      std::cout << "typeid(m_dtGeometry) == typeid(m_gpr_fitter.getDTGeometry()): " << (typeid(m_dtGeometry) == typeid(m_gpr_fitter.getDTGeometry())) << std::endl;
      std::cout << "typeid(*m_dtGeometry) == typeid(*m_gpr_fitter.getDTGeometry()): " << (typeid(m_dtGeometry) == typeid(m_gpr_fitter.getDTGeometry())) << std::endl;
  }
  std::cout << "Created empty GPR fitter!" << std::endl;

  // m_6DOF_GPR_container = new MuonResidualsTwoBin(m_twoBin, new MuonResiduals6DOFFitter(residualsModel, m_minAlignmentHits, useResiduals, m_weightAlignment),
  //                                       new MuonResiduals6DOFFitter(residualsModel, m_minAlignmentHits, useResiduals, m_weightAlignment));
  // std::cout << "Created standard 6 DOF fitter for GPR fit." << std::endl;
      ...
      if (m_minTrackPt < track->pt() && track->pt() < m_maxTrackPt && m_minTrackP < track->p() &&
        ...
        if (fabs(track->dxy(eventInfo.beamSpot().position())) < m_maxDxy) {
          ...
          MuonResidualsFromTrack muonResidualsFromTrack(builder,
                                                        ...
                                                        1000.,
                                                        m_createLayerNtuple_DT,
                                                        m_createLayerNtuple_CSC,
                                                        &layerData_DT,
                                                        m_ttree_DT_layers,
                                                        &layerData_CSC,
                                                        m_ttree_CSC_layers);
                  ...
                  if (dt2->numHits() >= m_minDT2Hits) {
                    ...
                    if (fitter != m_fitters.end()) {
                      ...
                      if (fabs(dt2->resslope()) < m_maxResSlopeY && (dt2->chi2() / double(dt2->ndof())) < 2.0) {
                        ...
                        residdata[MuonResiduals6DOFFitter::kWeightOccupancy] =
                            myOccupancyMap.GiveCorrection(DTChamberId(chamberId->rawId()).wheel(),
                                                          DTChamberId(chamberId->rawId()).station(),
                                                          DTChamberId(chamberId->rawId()).sector(),
                                                          dt13->trackx(),
                                                          dt13->tracky());
                        if (m_debug) {
              ...
              if (dt13 != nullptr) {
                ...
                if (dt13->numHits() >= m_minDT13Hits) {
                  ...
                  if (fitter != m_fitters.end()) {
                    ...
                    residdata[MuonResiduals5DOFFitter::kWeightOccupancy] =
                        myOccupancyMap.GiveCorrection(DTChamberId(chamberId->rawId()).wheel(),
                                                      DTChamberId(chamberId->rawId()).station(),
                                                      DTChamberId(chamberId->rawId()).sector(),
                                                      dt13->trackx(),
...
void MuonAlignmentFromReference::fitAndAlign() {
  ...
  if (m_debug)
    ...
    // std::cout << "Created empty GPR fitter!" << std::endl;
    //
    // std::cout << "Contents of m_fitters:" << '\n';
    // std::cout << "m_fitters.size = " << m_fitters.size() << std::endl;
    ...
    for (const auto& ali: m_alignables) 
    {
        ...
        if (id.subdetId() == MuonSubdetId::DT) {
            ...
            std::cout << "DT: " << dtId.wheel() << "/"
                                << dtId.station() << "/"
                                << dtId.sector() << std::endl;
            std::map<Alignable*, MuonResidualsTwoBin*>::const_iterator it = m_fitters.find(ali);
            m_gpr_fitter.fill(it);
            std::cout << "m_gpr_fitter.getSize() = " << m_gpr_fitter.getSize() << std::endl;
            std::cout << "Printing first ten dX residuals from m_gpr_fitter:" << std::endl;
            int count1 = 0;
            for (std::vector<double *>::const_iterator it = m_gpr_fitter.selected_chamber_residualsPos_begin(ali);
                                                       it != m_gpr_fitter.selected_chamber_residualsPos_end(ali);
                                                       ++it)
            {
                if (count1 < 10)
                {
                    if (dtId.station() != 4)
                    {
                        std::cout << (*it)[static_cast<int>(MuonResidualsGPRFitter::Data_6DOF::kResidX)] << " ";
                    }
                    else
                    {
                        std::cout << "It's station 4; enum is not ready." << std::endl;
                        break;
                    }
                }
                count1++;
            ...
            std::cout << std::endl;
            std::cout << "numResidualsPos = " << m_fitters[ali]->numResidualsPos() << std::endl;
            std::cout << "numResidualsNeg = " << m_fitters[ali]->numResidualsNeg() << std::endl;
            std::cout << "npar = " << m_fitters[ali]->npar() << std::endl;
            std::cout << "ndata = " << m_fitters[ali]->ndata() << std::endl;
            std::cout << "type = " << m_fitters[ali]->type() << std::endl;
            std::cout << "useRes = " << m_fitters[ali]->useRes() << std::endl;
            int count = 0;
            if (dtId.station() != 4) std::cout << "MuonResiduals6DOFFitter::kNData = " << MuonResiduals6DOFFitter::kNData << std::endl;
            else std::cout << "MuonResiduals5DOFFitter::kNData = " << MuonResiduals5DOFFitter::kNData << std::endl;
            std::cout<< "first ten dX residuals:" << std::endl;
            for (std::vector<double *>::const_iterator it = m_fitters[ali]->residualsPos_begin(); it != m_fitters[ali]->residualsPos_end(); ++it)
            {
                if (count < 10)
                {
                    if (dtId.station() != 4)
                    {
                        std::cout << (*it)[MuonResiduals6DOFFitter::kResidX] << " ";
                    }
                    else
                    {
                        std::cout << (*it)[MuonResiduals5DOFFitter::kResid] << " ";
                    }
                }
                count++;
            }
            if (dtId.station() == 1)
            {
                global_pos_count += count;
            }
            std::cout << std::endl;
            std::cout << "count for " << dtId <<" :" << count << std::endl;
            std::cout << "*****************" << std::endl;
        }
        if (id.subdetId() == MuonSubdetId::CSC) {
            CSCDetId cscId(id.rawId());
            std::cout << "CSC: " << cscId.zendcap() << "/"
                            << cscId.station() << "/"
                            << cscId.ring() << "/"
    ...
    // std::cout << "======End of m_fitters info=======" << std::endl;

    // std::cout << "===============Begin GPR fit==============" << std::endl;

    // values of gpr shift in local coordinates
    // AlgebraicVector gpr_params(6);
    // std::cout << "Checking m_6DOF_GPR_container:" << std::endl;
    // fill standard 6DOF fitter for gpr here
    // bool found_first_DT = false;
    // Alignable* first_DT;
    // for (std::map<Alignable*, MuonResidualsTwoBin*>::const_iterator iter = m_fitters.begin(); iter != m_fitters.end(); ++iter)
    // {
    //     Alignable* cur_ali = iter->first;
    //     DetId curDetId = cur_ali->geomDetId();
    //     if (curDetId.subdetId() == MuonSubdetId::DT)
    //     {
    //         if (!found_first_DT)
    //         {
    //             first_DT = cur_ali;
    //             found_first_DT = true;
    //         }
    //         DTChamberId curDTid(curDetId.rawId());
    //         if (curDTid.station() == 1)
    //         {
    //             MuonResidualsTwoBin* curTwoBin = iter->second;
    //             for (std::vector<double *>::const_iterator pos_iter = curTwoBin->residualsPos_begin();
    //                                                        pos_iter != curTwoBin->residualsPos_end();
    //                                                        ++pos_iter)
    //             {
    //                 m_6DOF_GPR_container->fill((*pos_iter)[MuonResiduals6DOFFitter::kCharge], *pos_iter);
    //             }
    //         }
    //         else continue;
    //     }
    //     else continue;
    // }
    // std::cout << "Total positive residual counter for station 1 numResidualsPos = " << m_6DOF_GPR_container->numResidualsPos() << std::endl;
    // std::cout << "Total negative residual counter for station 1 numResidualsNeg = " << m_6DOF_GPR_container->numResidualsNeg() << std::endl;
    // std::cout << "Total positive residual counter for station 1 global_pos_count = " << global_pos_count << std::endl;

    // std::cout << "Fit all alignables in m_6DOF_GPR_container:" << std::endl;
    // bool successful_gpr_fit = false;
    // if (first_DT)
    // {
    //     successful_gpr_fit = m_6DOF_GPR_container->fit(first_DT);
    // }
    // else
    // {
    //     std::cout << "first_DT = nullptr" << std::endl;
    // }
    // if (successful_gpr_fit)
    // {
    //     double gpr_deltax_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignX);
    //     double gpr_deltay_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignY);
    //     double gpr_deltaz_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignZ);
    //     double gpr_deltaphix_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiX);
    //     double gpr_deltaphiy_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiY);
    //     double gpr_deltaphiz_value = m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiZ);

    //     gpr_params[0] = gpr_deltax_value;
    //     gpr_params[1] = gpr_deltay_value;
    //     gpr_params[2] = gpr_deltaz_value;
    //     gpr_params[3] = gpr_deltaphix_value;
    //     gpr_params[4] = gpr_deltaphiy_value;
    //     gpr_params[5] = gpr_deltaphiz_value;

    //     std::cout << "Succesful GPR fit; Extracting GPR parameters:" << std::endl;
    //     std::cout << "gpr_deltax_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignX) << std::endl;
    //     std::cout << "gpr_deltay_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignY) << std::endl;
    //     std::cout << "gpr_deltaz_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignZ) << std::endl;
    //     std::cout << "gpr_deltaphix_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiX) << std::endl;
    //     std::cout << "gpr_deltaphiy_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiY) << std::endl;
    //     std::cout << "gpr_deltaphiz_value = " << m_6DOF_GPR_container->value(MuonResiduals6DOFFitter::kAlignPhiZ) << std::endl;
    //     std::cout << "=============End of GPR fit==============" << std::endl;
    // }
  ...
  for (const auto& ali : m_alignables) {
    ...
    if (id_check.subdetId() == MuonSubdetId::DT) {
      ...
      std::cout << "Current Alignable is DT" << std::endl;
      std::cout << "Current ALignable ID (DT chamber ID): " << chamberId_check.wheel() << "/" << chamberId_check.station() << "/" << chamberId_check.sector() << std::endl;
    ...
    if (id_check.subdetId() == MuonSubdetId::CSC) {
      ...
      if (chamberId_check.station() == 1 && chamberId_check.ring() == 3)
        WannaUsenoY = true;
    }
    ...
    if (m_combineME11 &&
        id.subdetId() ==
            MuonSubdetId::
                CSC) {  // ME 1/1 and ME 1/4 are the same chambers but are divided. here you are fitting them as a single chamber.
      CSCDetId cscid(id.rawId());
        ...
        if (fitter->second->type() == MuonResidualsFitter::k5DOF) {
          ...
          if (writeReport) {
            ...
            for (int ii = 0; ii < 64; ii++) {
              if (ii != 63)
                report << "'" << fitter->second->CovMatr().GetMatrixArray()[ii] << "',";
              else
                report << "'" << fitter->second->CovMatr().GetMatrixArray()[ii] << "'";
            }
            report << "]" << std::endl;
...
void MuonAlignmentFromReference::writeTmpFiles() {
  ...
  int size = m_indexes.size();          // m_indexes is a vector of int. Each int is the Raw ID of the chamber.
  fwrite(&size, sizeof(int), 1, file);  //WRITE the size of m_indexes

  int i = 0;
  for (std::vector<unsigned int>::const_iterator index = m_indexes.begin(); index != m_indexes.end(); ++index, ++i) {
    MuonResidualsTwoBin* fitter = m_fitterOrder[*index];
    unsigned int index_towrite = *index;
    fwrite(&index_towrite, sizeof(unsigned int), 1, file);  // WRITE the RAW_ID of the chamber (one by one)
    fitter->write(
        file,
        i);  //All the other informations are written inside this method (you just need the file and the index to the element to write)
  }
  ...
  for (std::vector<unsigned int>::const_iterator index = m_indexes.begin(); index != m_indexes.end(); ++index) {
    ...
    for (; residual != fitter->residualsPos_end(); ++residual, ++residual_ok) {
      if (fitter->type() == MuonResidualsFitter::k5DOF || fitter->type() == MuonResidualsFitter::k6DOFrphi) {
        ...
        m_tree_row.OccuWeight = (fitter->type() == MuonResidualsFitter::k5DOF)
                                    ? (Float_t)(*residual)[MuonResiduals5DOFFitter::kWeightOccupancy]
                                    : -1.;  // for CSC you do not have this element in the array
